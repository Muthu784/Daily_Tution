  ...
  return <ComponentContainerCard title='Floating Labels' description={<>Wrap a pair of <code>&lt;input class=&quot;form-control&quot;&gt;</code> and <code>&lt;label&gt;</code> elements in <code>.form-floating</code> to enable floating labels with Bootstrap’s textual form fields. A <code>placeholder</code> is required on each <code>&lt;input&gt;</code> as our method of CSS-only floating labels uses the <code>:placeholder-shown</code> pseudo-element. Also note that the <code>&lt;input&gt;</code> must come first so we can utilize a sibling selector (e.g., <code>~</code>).</>}>
      <Row>
        <Col lg={6}>
          ...
          <div className="form-floating">
            <Form.Control as={'textarea'} placeholder="Leave a comment here" id="floatingTextarea" style={{
            height: 100
          }} defaultValue={""} />
...
const SelectForm = () => {
  return <ComponentContainerCard title='Select' description={<><code>&lt;select&gt;</code> menus need only a custom class, <code>.form-select</code> to trigger the custom styles.</>}>
...
const SwitchesForm = () => {
  return <ComponentContainerCard title='Switches' description={<>A switch has the markup of a custom checkbox but uses the <code>.form-switch</code> class to render a toggle switch. Switches also support the <code>disabled</code> attribute.</>}>
...
const CheckboxesForm = () => {
  return <ComponentContainerCard title='Checkboxes' description={<>Each checkbox and radio <code>&lt;input&gt;</code> and <code>&lt;label&gt;</code> pairing is wrapped in a <code>&lt;div&gt;</code> to create our custom control. Structurally, this is the same approach as our default <code>.form-check</code>.</>}>
      <div className="mt-3">
...
const Radios = () => {
  return <ComponentContainerCard title='Radios' description={<>Each checkbox and radio <code>&lt;input&gt;</code> and <code>&lt;label&gt;</code> pairing is wrapped in a <code>&lt;div&gt;</code> to create our custom control. Structurally, this is the same approach as our default <code>.form-check</code>.</>}>
      <div className="mt-3">
        <div className="form-check mb-1">
...
const InputSizes = () => {
  return <ComponentContainerCard title='Input Sizes' description={<> Set heights using classes like <code>.input-lg</code>, and set widths using grid column classes like <code>.col-lg-*</code>.</>}>
      <Form>
        <div className="mb-3">
...
const InputGroup = () => {
  return <ComponentContainerCard title='Input Group' description={<> Easily extend form controls by adding text, buttons, or button groups on either side of textual inputs, custom selects, and custom file inputs</>}>
      <form>
        <div className="mb-3">
...
const CustomSwitch = () => {
  return <ComponentContainerCard title='Custom Switch' description={<>Here are a few types of switches.</>}>
