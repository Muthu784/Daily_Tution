...
const BasicForm = () => {
  return <ComponentContainerCard title='Basic Example' description={<>Here’s a quick example to demonstrate Bootstrap’s form styles. Keep reading for documentation on required classes, form layout, and more.</>}>
      <Form>
        <div className="mb-3">
...
const HorizontalSizingForm = () => {
  return <ComponentContainerCard title='Horizontal form label sizing' description={<>Be sure to use <code>.col-form-label-sm</code> or <code>.col-form-label-lg</code> to your <code>&lt;label&gt;</code>s or <code>&lt;legend&gt;</code>s to correctly follow the size of <code>.form-control-lg</code> and <code>.form-control-sm</code>.</>}>
      <form>
        <Row className="mb-2">
...
const FormRow = () => {
  return <ComponentContainerCard title='Form Row' description={<>By adding <code>.row</code> &amp; <code>.g-2</code>, you can have control over the gutter width in as well the inline as block direction.</>}>
      <form>
        <Row className="g-2">
